...
// pub const BETA: usize = 43; // pretty sure i got this right
...
impl SealSig{
    ...
    fn get_constraints(rlen: usize, ilen: usize, olen: usize, u: &Scalar, v: &Scalar, y: &Scalar, z: &Scalar) -> (Vec<Scalar>, Vec<Scalar>, Vec<Scalar>, Vec<Scalar>, Vec<Scalar>, Vec<Scalar>, Vec<Scalar>, Scalar) {
        ...
        let m = 3 + rlen + rlen*ilen + olen*BETA + 3 * ilen;

        let mut v0 = vec![Scalar::zero(); 3+rlen];
        let yexps: Vec<Scalar> = exp_iter(*y).take(rlen*ilen+BETA*olen).collect();
        v0.extend(yexps);
        v0.extend(vec![Scalar::zero(); 3*ilen]);

        let mut v1 = vec![Scalar::zero();  3 + rlen + rlen*ilen + olen*BETA + 2*ilen];
        ...
        let twoexp: Vec<Scalar> = exp_iter(Scalar::from(2u8)).take(BETA).collect();
        ...
        v3.extend(vec![Scalar::zero(); BETA*olen + 3 * ilen]);
        v3[2] = yexps[yexps.len()-1];

        let mut v4 = vec![Scalar::one()];
        v4.extend(vec![Scalar::zero(); 2+rlen+ rlen*ilen+ BETA*olen]);
        let vexps: Vec<Scalar> = exp_iter(*v).take(ilen).map(|vexp| u*vexp).collect();
        v4.extend( vexps);
        v4.extend(vec![Scalar::zero(); 2 * ilen]);

        let mut v5 = vec![Scalar::zero(); 3+rlen+ rlen*ilen+ BETA*olen + ilen];
        ...
        v6.extend(vec![Scalar::zero(); olen*BETA + 3 * ilen]);
        ...
        let mut u4 = vec![Scalar::zero(); 3 + rlen + rlen*ilen + olen*BETA + 2 * ilen];
    ...
    pub fn verify(&self, transcript: &mut Transcript, ring: &[&OTAccount], tags: &[&Tag], outputs: &[&OTAccount]) -> Result<(), SealError> {
        ...
        let mut G_factors: Vec<Scalar> = iter::repeat(Scalar::one()).take(m).collect();
        G_factors.par_extend(vec![Scalar::zero(); padlen]);
        let mut H_factors: Vec<Scalar> = theta_inv;
        H_factors.par_extend(vec![Scalar::zero(); padlen]);
